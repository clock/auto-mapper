    ...
    public static void download(String version, File destination) {
        try {
            ...
            try (BufferedInputStream in = new BufferedInputStream(getURLInputStream(new URL(url))); FileOutputStream out = new FileOutputStream(temp)) {
                ...
                ZipEntry params = zipFile.getEntry("params.csv");
                ...
                InputStream paramsIn = zipFile.getInputStream(params);

                FileOutputStream paramsFos = new FileOutputStream(new File(destination, "params.csv"));

                while ((read = paramsIn.read(buffer, 0, 1024)) != -1) {
                    paramsFos.write(buffer, 0, read);
                }
            }
            ...
            try (BufferedInputStream in = new BufferedInputStream(getURLInputStream(new URL(srgUrl))); FileOutputStream out = new FileOutputStream(srgTemp)) {
                ...
                FileOutputStream joinedFos = new FileOutputStream(new File(destination, version + ".srg"));
